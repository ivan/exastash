...
CREATE UNIQUE INDEX google_domains_domain_index ON google_domains (domain);

CREATE TRIGGER google_domains_check_update
    BEFORE UPDATE ON google_domains
    FOR EACH ROW
    WHEN (OLD.id != NEW.id)
    EXECUTE FUNCTION raise_exception('cannot change id');

CREATE TRIGGER google_domains_forbid_truncate
    BEFORE TRUNCATE ON google_domains
    EXECUTE FUNCTION raise_exception('truncate is forbidden');
...
CREATE TABLE gdrive_owners (
    ...
    domain  smallint  NOT NULL REFERENCES google_domains(id),
...
-- Where to place new Google Drive files

CREATE TABLE gdrive_file_placement (
    domain  smallint  NOT NULL REFERENCES google_domains(id),
    owner   int       NOT NULL REFERENCES gdrive_owners(id),
    parent  text      NOT NULL REFERENCES gdrive_parents(name),

    PRIMARY KEY (domain, owner, parent)
);

CREATE TRIGGER gdrive_file_placement_check_update
    BEFORE UPDATE ON gdrive_file_placement
    FOR EACH ROW
    EXECUTE FUNCTION raise_exception('cannot change domain, owner, or parent');

CREATE TRIGGER gdrive_file_placement_forbid_truncate
    BEFORE TRUNCATE ON gdrive_file_placement
    EXECUTE FUNCTION raise_exception('truncate is forbidden');
...
CREATE TABLE storage_gdrive (
    ...
    google_domain  smallint  NOT NULL REFERENCES google_domains (id),
    ...
    PRIMARY KEY (file_id, google_domain, cipher)
);
...
CREATE TRIGGER storage_gdrive_check_update
    ...
    FOR EACH ROW
    EXECUTE FUNCTION raise_exception('cannot change file_id, google_domain, cipher, cipher_key, or gdrive_ids');
