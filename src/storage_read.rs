...
fn stream_gdrive_files(file: &inode::File, storage: &gdrive::Storage) -> impl Stream<Item = Result<Bytes, Error>> {
    let file = file.clone();
    let storage = storage.clone();

    assert!(storage.cipher == gdrive::Cipher::Aes128Gcm);
...
pub async fn read(file: &inode::File, storage: &Storage) -> Result<Pin<Box<dyn Stream<Item = Result<Bytes, Error>>>>> {
    ...
    Ok(match storage {
        ...
        Storage::Gdrive(gdrive_storage) => {
            Box::pin(stream_gdrive_files(file, gdrive_storage))
        }
